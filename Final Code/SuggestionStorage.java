...
public class SuggestionStorage implements Storage {
    ...
    public void printData(String campName) {
        ...
        for (Suggestion suggestion : suggestions) {
            if (suggestion.getCampName().equals(campName) && !suggestion.getSuggestion().isEmpty()){
                ...
                System.out.println(" Suggester: " + suggestion.getSuggester());
                System.out.println(" Suggestion: " + suggestion.getSuggestion());
                String processStatus = suggestion.getProcessed() ? "Processed" : "Not Processed";
                String approvalStatus = suggestion.getApproval() ? "Approved" : "Rejected";
                System.out.println(" Processing Status: " + processStatus);
    ...
    /**
     * To print the data of the all suggestions for a certain student.
     * @param studentAccount The StudentAccount object.
     */
    public void printData(StudentAccount studentAccount) {
        String studentId = studentAccount.getUserId();
        boolean noSuggestion = true;
        for (Suggestion suggestion : suggestions){
            if(suggestion.getSuggester().equals(studentId)) {
                noSuggestion = false;
                System.out.println("[ Suggestion ID " + suggestion.getSuggestionId() + " ]");
                System.out.println(" Suggestion: " + suggestion.getSuggestion());
                String processStatus = suggestion.getProcessed() ? "Processed" : "Not Processed";
